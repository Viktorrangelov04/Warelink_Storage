...
export default function InventoryPage() {
  ...
  const loadCategories = async () => {
    ...
    const namesOnly = fetchedCategories.map((cat) => cat.name);
      ...
      <tr key={product.id}>
        ...
        <td
          className={`border px-4 py-2 ${
            product.quantity < lowStockThreshold ? "bg-red-500 font-bold" : ""
          }`}
        >
          {product.quantity}
        </td>
        <td className="border px-4 py-2 text-center">
          <button onClick={() => openEditModal(product)} className="mr-2">
            ✎
          </button>
